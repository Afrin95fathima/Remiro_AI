#!/usr/bin/env python3
"""
Test both career insights and action plan with the correct data structure
"""
import json
import os
import asyncio
...
# Define MasterCareerAgent class (with correct data access patterns)
class MasterCareerAgent:
    ...
    async def generate_insights(self, user_profile):
        ...
        try:
            # Try both data structures - new format uses 'data', old format uses 'assessments'
            data = user_profile.get('data', user_profile.get('assessments', {}))
            ...
            completed_assessments = sum(1 for key, value in data.items() 
                                     if key != 'username' and isinstance(value, dict) and 
                                     (value.get('summary') or (isinstance(value.get('data'), dict) and value['data'].get('summary'))))
            
            print(f"📊 Found {completed_assessments} completed assessments")
    ...
    async def generate_action_plan(self, user_profile):
        ...
        try:
            ...
            print("🔄 Generating action plan with Gemini...")
            # Generate action plan
            response = await self.llm.ainvoke(action_plan_prompt)
            action_plan_text = response.content.strip()
            
            print(f"📝 Raw response length: {len(action_plan_text)} characters")
...
try:
    ...
    if 'assessments' in user_data:
        assessments_keys = list(user_data['assessments'].keys())
        print(f"📋 Assessment types: {assessments_keys}")
        
        # Count assessments
        completed_count = sum(1 for key, value in user_data['assessments'].items() 
                             if isinstance(value, dict) and 
                             (value.get('summary') or (isinstance(value.get('data'), dict) and value['data'].get('summary'))))
        print(f"✅ Completed assessments: {completed_count}/12")
    else:
        print("❌ No 'assessments' key found in profile")
...
if insights_success and action_plan_success:
    ...
    print("\n✨ **SUMMARY**: The fixes have resolved the issues!")
    print("- Data access patterns now correctly handle both 'data' and 'assessments' keys")
    print("- Error handling is comprehensive and provides useful debugging info")  
    print("- JSON structures are simplified for better reliability")
    print("- Career insights generation is working properly")
    print("- Action plan generation is working properly")
